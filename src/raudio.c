...
void InitAudioDevice(void)
{
    ...
    {
        TRACELOG(LOG_LEVEL_WARNING, "AUDIO: Failed to initialize context");
        return;
    }
    ...
    if (result != MA_SUCCESS)
    {
        TRACELOG(LOG_LEVEL_WARNING, "AUDIO: Failed to initialize playback device");
    ...
    {
        TRACELOG(LOG_LEVEL_WARNING, "AUDIO: Failed to create mutex for mixing");
    ...
    if (result != MA_SUCCESS)
    {
        TRACELOG(LOG_LEVEL_WARNING, "AUDIO: Failed to start playback device");
        ma_device_uninit(&AUDIO.System.device);
        ma_context_uninit(&AUDIO.System.context);
        return;
    }

    TRACELOG(LOG_LEVEL_INFO, "AUDIO: Device initialized successfully");
    TRACELOG(LOG_LEVEL_INFO, "    > Backend:       miniaudio | %s", ma_get_backend_name(AUDIO.System.context.backend));
    TRACELOG(LOG_LEVEL_INFO, "    > Format:        %s -> %s", ma_get_format_name(AUDIO.System.device.playback.format), ma_get_format_name(AUDIO.System.device.playback.internalFormat));
    TRACELOG(LOG_LEVEL_INFO, "    > Channels:      %d -> %d", AUDIO.System.device.playback.channels, AUDIO.System.device.playback.internalChannels);
    TRACELOG(LOG_LEVEL_INFO, "    > Sample rate:   %d -> %d", AUDIO.System.device.sampleRate, AUDIO.System.device.playback.internalSampleRate);
    TRACELOG(LOG_LEVEL_INFO, "    > Periods size:  %d", AUDIO.System.device.playback.internalPeriodSizeInFrames*AUDIO.System.device.playback.internalPeriods);
...
void CloseAudioDevice(void)
{
    if (AUDIO.System.isReady)
    {
        ...
        TRACELOG(LOG_LEVEL_INFO, "AUDIO: Device closed successfully");
    }
    else TRACELOG(LOG_LEVEL_WARNING, "AUDIO: Device could not be closed, not currently initialized");
}
...
AudioBuffer *LoadAudioBuffer(ma_format format, ma_uint32 channels, ma_uint32 sampleRate, ma_uint32 sizeInFrames, int usage)
{
    ...
    if (audioBuffer == NULL)
    {
        TRACELOG(LOG_LEVEL_WARNING, "AUDIO: Failed to allocate memory for buffer");
        return NULL;
    }
    ...
    if (result != MA_SUCCESS)
    {
        TRACELOG(LOG_LEVEL_WARNING, "AUDIO: Failed to create data conversion pipeline");
        RL_FREE(audioBuffer);
        return NULL;
    }
...
Wave LoadWaveFromMemory(const char *fileType, const unsigned char *fileData, int dataSize)
{
    ...
    else if ((strcmp(fileType, ".wav") == 0) || (strcmp(fileType, ".WAV") == 0))
    {
        ...
        else TRACELOG(LOG_LEVEL_WARNING, "WAVE: Failed to load WAV data");
    ...
    else if ((strcmp(fileType, ".ogg") == 0) || (strcmp(fileType, ".OGG") == 0))
    {
        ...
        else TRACELOG(LOG_LEVEL_WARNING, "WAVE: Failed to load OGG data");
    }
#endif
#if SUPPORT_FILEFORMAT_MP3
    else if ((strcmp(fileType, ".mp3") == 0) || (strcmp(fileType, ".MP3") == 0))
    {
        ...
        else TRACELOG(LOG_LEVEL_WARNING, "WAVE: Failed to load MP3 data");
    ...
    else if ((strcmp(fileType, ".qoa") == 0) || (strcmp(fileType, ".QOA") == 0))
    {
        ...
        else TRACELOG(LOG_LEVEL_WARNING, "WAVE: Failed to load QOA data");
    ...
    else if ((strcmp(fileType, ".flac") == 0) || (strcmp(fileType, ".FLAC") == 0))
    {
        ...
        else TRACELOG(LOG_LEVEL_WARNING, "WAVE: Failed to load FLAC data");
    }
#endif
    else TRACELOG(LOG_LEVEL_WARNING, "WAVE: Data format not supported");

    TRACELOG(LOG_LEVEL_INFO, "WAVE: Data loaded successfully (%i Hz, %i bit, %i channels)", wave.sampleRate, wave.sampleSize, wave.channels);
...
Sound LoadSoundFromWave(Wave wave)
{
    ...
    if (wave.data != NULL)
    {
        ...
        if (frameCount == 0) TRACELOG(LOG_LEVEL_WARNING, "SOUND: Failed to get frame count for format conversion");
        ...
        if (audioBuffer != NULL)
        {
            ...
            if (frameCount == 0) TRACELOG(LOG_LEVEL_WARNING, "SOUND: Failed format conversion");
        ...
        else TRACELOG(LOG_LEVEL_WARNING, "SOUND: Failed to create buffer");
    }
...
{
    RL_FREE(wave.data);
    //TRACELOG(LOG_LEVEL_INFO, "WAVE: Unloaded wave data from RAM");
}
...
void UnloadSound(Sound sound)
{
    ...
    //TRACELOG(LOG_LEVEL_INFO, "SOUND: Unloaded sound data from RAM");
}
...
bool ExportWave(Wave wave, const char *fileName)
{
    ...
    else if (IsFileExtension(fileName, ".qoa"))
    {
        ...
        else TRACELOG(LOG_LEVEL_WARNING, "AUDIO: Wave data must be 16 bit per sample for QOA format export");
    }
#endif
    ...
    if (result) TRACELOG(LOG_LEVEL_INFO, "FILEIO: [%s] Wave data exported successfully", fileName);
    else TRACELOG(LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to export wave data", fileName);
...
bool ExportWaveAsCode(Wave wave, const char *fileName)
{
    ...
    if (result != 0) TRACELOG(LOG_LEVEL_INFO, "FILEIO: [%s] Wave as code exported successfully", fileName);
    else TRACELOG(LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to export wave as code", fileName);
...
void WaveFormat(Wave *wave, int sampleRate, int sampleSize, int channels)
{
    ...
    if (frameCount == 0)
    {
        TRACELOG(LOG_LEVEL_WARNING, "WAVE: Failed to get frame count for format conversion");
        return;
    }
    ...
    {
        RL_FREE(data);
        TRACELOG(LOG_LEVEL_WARNING, "WAVE: Failed format conversion");
        return;
    }
...
void WaveCrop(Wave *wave, int initFrame, int finalFrame)
{
    ...
    else TRACELOG(LOG_LEVEL_WARNING, "WAVE: Crop range out of bounds");
}
...
#endif
    else TRACELOG(LOG_LEVEL_WARNING, "STREAM: [%s] File format not supported", fileName);

    if (!musicLoaded)
    {
        TRACELOG(LOG_LEVEL_WARNING, "FILEIO: [%s] Music file could not be opened", fileName);
    }
    else
    {
        // Show some music stream info
        TRACELOG(LOG_LEVEL_INFO, "FILEIO: [%s] Music file loaded successfully", fileName);
        TRACELOG(LOG_LEVEL_INFO, "    > Sample rate:   %i Hz", music.stream.sampleRate);
        TRACELOG(LOG_LEVEL_INFO, "    > Sample size:   %i bits", music.stream.sampleSize);
        TRACELOG(LOG_LEVEL_INFO, "    > Channels:      %i (%s)", music.stream.channels, (music.stream.channels == 1)? "Mono" : (music.stream.channels == 2)? "Stereo" : "Multi");
        TRACELOG(LOG_LEVEL_INFO, "    > Total frames:  %i", music.frameCount);
    }
...
#endif
    else TRACELOG(LOG_LEVEL_WARNING, "STREAM: Data format not supported");

    if (!musicLoaded)
    {
        TRACELOG(LOG_LEVEL_WARNING, "FILEIO: Music data could not be loaded");
    }
    else
    {
        // Show some music stream info
        TRACELOG(LOG_LEVEL_INFO, "FILEIO: Music data loaded successfully");
        TRACELOG(LOG_LEVEL_INFO, "    > Sample rate:   %i Hz", music.stream.sampleRate);
        TRACELOG(LOG_LEVEL_INFO, "    > Sample size:   %i bits", music.stream.sampleSize);
        TRACELOG(LOG_LEVEL_INFO, "    > Channels:      %i (%s)", music.stream.channels, (music.stream.channels == 1)? "Mono" : (music.stream.channels == 2)? "Stereo" : "Multi");
        TRACELOG(LOG_LEVEL_INFO, "    > Total frames:  %i", music.frameCount);
...
AudioStream LoadAudioStream(unsigned int sampleRate, unsigned int sampleSize, unsigned int channels)
{
    ...
    if (stream.buffer != NULL)
    {
        ...
        TRACELOG(LOG_LEVEL_INFO, "STREAM: Initialized successfully (%i Hz, %i bit, %s)", stream.sampleRate, stream.sampleSize, (stream.channels == 1)? "Mono" : "Stereo");
    }
    else TRACELOG(LOG_LEVEL_WARNING, "STREAM: Failed to load audio buffer, stream could not be created");
...
void UnloadAudioStream(AudioStream stream)
{
    ...
    TRACELOG(LOG_LEVEL_INFO, "STREAM: Unloaded audio stream data from RAM");
}
...
static void OnLog(void *pUserData, ma_uint32 level, const char *pMessage)
{
    TRACELOG(LOG_LEVEL_WARNING, "miniaudio: %s", pMessage);   // All log messages from miniaudio are errors
}
...
static void UpdateAudioStreamInLockedState(AudioStream stream, const void *data, int frameCount)
{
    if (stream.buffer != NULL)
    {
        if (stream.buffer->isSubBufferProcessed[0] || stream.buffer->isSubBufferProcessed[1])
        {
            ...
            else TRACELOG(LOG_LEVEL_WARNING, "STREAM: Attempting to write too many frames to buffer");
        }
        else TRACELOG(LOG_LEVEL_WARNING, "STREAM: Buffer not available for updating");
    }
}
        ...
        if (file != NULL)
        {
            ...
            if (size > 0)
            {
                ...
                *dataSize = count;
                ...
                if (count != size) TRACELOG(LOG_LEVEL_WARNING, "FILEIO: [%s] File partially loaded", fileName);
...
static bool SaveFileData(const char *fileName, void *data, int dataSize)
{
    ...
    if (fileName != NULL)
    {
        ...
        {
            TRACELOG(LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to open file", fileName);
            result = false;
        }
    }
    else
    {
        TRACELOG(LOG_LEVEL_WARNING, "FILEIO: File name provided is not valid");
        result = false;
    }
...
static bool SaveFileText(const char *fileName, char *text)
{
    ...
    if (fileName != NULL)
    {
        ...
        {
            TRACELOG(LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to open text file", fileName);
            result = false;
        }
    }
    else

...
void OpenURL(const char *url)
{
    ...
    if ((strchr(url, '\'') != NULL) || (strchr(url, '\"') != NULL))
    {
        ...
        TRACELOG(LOG_LEVEL_WARNING, "SYSTEM: Provided URL could be potentially malicious, avoid [\'\"] characters");
    }
    else if ((strncmp(url, "http://", 7) != 0) && (strncmp(url, "https://", 8) != 0))
    {
        // Only allow URL starting with "http://" or "https://" protocols
        TRACELOG(LOG_LEVEL_WARNING, "SYSTEM: Provided URL must start with 'http://' or 'https://' protocols");
    }

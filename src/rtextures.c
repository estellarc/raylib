...
// Image fileformats not supported by default
    ...
    #define RLTEXGPU_LOG(...) TRACELOG(LOG_LEVEL_WARNING, "IMAGE: " __VA_ARGS__)
    #define RLTEXGPU_SHOW_LOG_LEVEL_INFO
...
Image LoadImageFromMemory(const char *fileType, const unsigned char *fileData, int dataSize)
{
    ...
    {
        TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Invalid file data");
        return image;
    }
    if (fileType == NULL)
    {
        TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Missing file extension");
        return image;
    }
    ...
    else if ((strcmp(fileType, ".hdr") == 0) || (strcmp(fileType, ".HDR") == 0))
    {
        ...
        if (fileData != NULL)
        {
            ...
            {
                TRACELOG(LOG_LEVEL_WARNING, "IMAGE: HDR file format not supported");
                UnloadImage(image);
            }
        }
#endif
...
#endif
    else TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Data format not supported");

    if (image.data != NULL) TRACELOG(LOG_LEVEL_INFO, "IMAGE: Data loaded successfully (%ix%i | %s | %i mipmaps)", image.width, image.height, rlGetPixelFormatName(image.format), image.mipmaps);
    else TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Failed to load image data");
...
Image LoadImageFromTexture(Texture2D texture)
{
    ...
    if (texture.format < PIXELFORMAT_COMPRESSED_DXT1_RGB)
    {
        ...
        if (image.data != NULL)
        {
            ...
            TRACELOG(LOG_LEVEL_INFO, "TEXTURE: [ID %i] Pixel data retrieved successfully", texture.id);
        }
        else TRACELOG(LOG_LEVEL_WARNING, "TEXTURE: [ID %i] Failed to retrieve pixel data", texture.id);
    }
    else TRACELOG(LOG_LEVEL_WARNING, "TEXTURE: [ID %i] Failed to retrieve compressed pixel data", texture.id);
...
bool ExportImage(Image image, const char *fileName)
{
    ...
    else if (IsFileExtension(fileName, ".qoi"))
    {
        ...
        else TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Image pixel format must be R8G8B8 or R8G8B8A8");
    ...
    else if (IsFileExtension(fileName, ".pep"))
    {
        ...
        else TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Image can not be exported, .pep requires RGBA 32bit input");
    }
#endif
#if SUPPORT_FILEFORMAT_KTX
    ...
    else TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Export image format requested not supported");

    if (allocatedData) RL_FREE(imgData);
#endif // SUPPORT_IMAGE_EXPORT

    if (result != 0) TRACELOG(LOG_LEVEL_INFO, "FILEIO: [%s] Image exported successfully", fileName);
    else TRACELOG(LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to export image", fileName);
...
#else
    TRACELOG(LOG_LEVEL_WARNING, "IMAGE: To export image, enable flag SUPPORT_IMAGE_EXPORT");
#endif
...
Image ImageFromImage(Image image, Rectangle rec)
{
    ...
    if (image.format < PIXELFORMAT_COMPRESSED_DXT1_RGB)
	{
        ...
        else TRACELOG(LOG_LEVEL_WARNING, "IMAGE: ImageToImage(), rectangle provided not valid");
	}
    else TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Image manipulation not supported for compressed formats");
...
void ImageCrop(Image *image, Rectangle crop)
{
    ...
    {
        TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Failed to crop, rectangle out of bounds");
        return;
    }

    if (image->mipmaps > 1) TRACELOG(LOG_LEVEL_WARNING, "Image manipulation only applied to base mipmap level");
    if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB) TRACELOG(LOG_LEVEL_WARNING, "Image manipulation not supported for compressed formats");
    else
    {
        int bytesPerPixel = GetPixelDataSize(1, 1, image->format);
...
void ImageFormat(Image *image, int newFormat)
{
    ...
    if ((newFormat != 0) && (image->format != newFormat))
    {
        ...
        else TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Data format is compressed, can not be converted");
    }
}
...
Image ImageText(const char *text, int fontSize, Color color)
{
    ...
    TRACELOG(LOG_LEVEL_WARNING, "IMAGE: ImageTextEx() requires module: rtext");
#endif
    return imText;
}
...
Image ImageTextEx(Font font, const char *text, float fontSize, float spacing, Color tint)
{
    ...
    if (textSize.y != imSize.y)
    {
        ...
        TRACELOG(LOG_LEVEL_INFO, "IMAGE: Text scaled by factor: %f", scaleFactor);
...
Image ImageFromChannel(Image image, int selectedChannel)
{
    ...
    {
        TRACELOG(LOG_LEVEL_WARNING, "Channel cannot be negative. Setting channel to 0.");
        selectedChannel = 0;
    }
        ...
        image.format == PIXELFORMAT_UNCOMPRESSED_R16)
    {
        if (selectedChannel > 0)
        {
            TRACELOG(LOG_LEVEL_WARNING, "This image has only 1 channel. Setting channel to it.");
            selectedChannel = 0;
        }
    }
    else if (image.format == PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA)
    {
        ...
        {
            TRACELOG(LOG_LEVEL_WARNING, "This image has only 2 channels. Setting channel to alpha.");
            selectedChannel = 1;
        }
    }
             ...
             image.format == PIXELFORMAT_UNCOMPRESSED_R16G16B16)
    {
        ...
        {
            TRACELOG(LOG_LEVEL_WARNING, "This image has only 3 channels. Setting channel to red.");
            selectedChannel = 0;
        }
    }
    ...
    {
        TRACELOG(LOG_LEVEL_WARNING, "ImageFromChannel supports channels 0 to 3 (RGBA). Setting channel to alpha.");
        selectedChannel = 3;
    }
    ...
    if (image.format >= PIXELFORMAT_COMPRESSED_DXT1_RGB) TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Pixel data retrieval not supported for compressed image formats");
    else
    {
...
void ImageAlphaMask(Image *image, Image alphaMask)
{
    ...
    {
        TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Alpha mask must be same size as image");
    }
    else if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    {
        TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Alpha mask can not be applied to compressed data formats");
    }
    else
    {
...
void ImageKernelConvolution(Image *image, const float *kernel, int kernelSize)
{
    ...
    {
        TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Convolution kernel must be square to be applied");
        return;
    }
...
void ImageMipmaps(Image *image)
{
    ...
    while ((mipWidth != 1) || (mipHeight != 1))
    {
        ...
        TRACELOG(LOG_LEVEL_DEBUG, "IMAGE: Next mipmap level: %i x %i - current size %i", mipWidth, mipHeight, mipSize);
    ...
    if (image->mipmaps < mipCount)
    {
        ...
        for (int i = 1; i < mipCount; i++)
        {
            ...
            TRACELOG(LOG_LEVEL_DEBUG, "IMAGE: Generating mipmap level: %i (%i x %i) - size: %i - offset: 0x%x", i, mipWidth, mipHeight, mipSize, nextmip);
    ...
    else TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Mipmaps already available");
}
...
void ImageDither(Image *image, int rBpp, int gBpp, int bBpp, int aBpp)
{
    ...
    if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    {
        TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Compressed data formats can not be dithered");
        return;
    }

    if ((rBpp + gBpp + bBpp + aBpp) > 16)
    {
        TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Unsupported dithering bpps (%ibpp), only 16bpp or lower modes supported", (rBpp+gBpp+bBpp+aBpp));
    }
    else
    {
        Color *pixels = LoadImageColors(*image);
        ...
        {
            TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Format is already 16bpp or lower, dithering could have no effect");
        }
        ...
        {
            image->format = 0;
            TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Unsupported dithered OpenGL internal format: %ibpp (R%iG%iB%iA%i)", (rBpp+gBpp+bBpp+aBpp), rBpp, gBpp, bBpp, aBpp);
        }
...
Color *LoadImageColors(Image image)
{
    ...
    else
    {
        if ((image.format == PIXELFORMAT_UNCOMPRESSED_R32) ||
            (image.format == PIXELFORMAT_UNCOMPRESSED_R32G32B32) ||
            (image.format == PIXELFORMAT_UNCOMPRESSED_R32G32B32A32)) TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Pixel format converted from 32bit to 8bit per channel");

        if ((image.format == PIXELFORMAT_UNCOMPRESSED_R16) ||
            (image.format == PIXELFORMAT_UNCOMPRESSED_R16G16B16) ||
            (image.format == PIXELFORMAT_UNCOMPRESSED_R16G16B16A16)) TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Pixel format converted from 16bit to 8bit per channel");
            ...
            if (pixels[i].a > 0)
            {
                ...
                if (!colorInPalette)
                {
                    ...
                    if (palCount >= maxPaletteSize)
                    {
                        ...
                        TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Palette is greater than %i colors", maxPaletteSize);
                    }
                }
            }
        }
...
Color GetImageColor(Image image, int x, int y)
{
    ...
    if ((x >=0) && (x < image.width) && (y >= 0) && (y < image.height))
    {
        switch (image.format)
        {
            ...
            default: TRACELOG(LOG_LEVEL_WARNING, "Compressed image format does not support color reading"); break;
        }
    }
    else TRACELOG(LOG_LEVEL_WARNING, "Requested image pixel (%i, %i) out of bounds", x, y);
...
void ImageDrawImagePro(Image *dst, Image src, Rectangle srcRec, Rectangle dstRec, Vector2 origin, float rotation, Color tint)
{
    ...
    if (dst->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB) TRACELOG(LOG_LEVEL_WARNING, "Image drawing not supported for compressed formats");
    else
    {
...
#else
    TRACELOG(LOG_LEVEL_WARNING, "IMAGE: ImageDrawText() requires module: rtext");
#endif
}
...
Texture2D LoadTextureFromImage(Image image)
{
    ...
    else TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Data is not valid to load texture");
...
TextureCubemap LoadTextureCubemap(Image image, int layout)
{
    ...
    if (layout != CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        else TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Failed to load cubemap image");

        UnloadImage(faces);
    }
    else TRACELOG(LOG_LEVEL_WARNING, "IMAGE: Failed to detect cubemap image layout");
...
RLAPI RenderTexture2D LoadRenderTextureEx(int width, int height, int format)
{
    ...
    {
        TRACELOG(LOG_LEVEL_WARNING, "FBO: Render texture format not supported");
        return target;
    }
    ...
    if (target.id > 0)
    {
        ...
        if (rlFramebufferComplete(target.id)) TRACELOG(LOG_LEVEL_INFO, "FBO: [ID %i] Framebuffer object created successfully", target.id);

        rlDisableFramebuffer();
    }
    else TRACELOG(LOG_LEVEL_WARNING, "FBO: Framebuffer object can not be created");
...
void UnloadTexture(Texture2D texture)
{
    if (texture.id > 0)
    {
        ...
        TRACELOG(LOG_LEVEL_INFO, "TEXTURE: [ID %i] Unloaded texture data from VRAM (GPU)", texture.id);
    }
}
...
void SetTextureFilter(Texture2D texture, int filter)
{
    switch (filter)
    {
        ...
        case TEXTURE_FILTER_TRILINEAR:
        {
            ...
            {
                TRACELOG(LOG_LEVEL_WARNING, "TEXTURE: [ID %i] No mipmaps available for TRILINEAR texture filtering", texture.id);

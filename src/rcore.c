...
void TraceLog(int logType, const char *text, ...)
{
    ...
    switch (logType)
    {
        case LOG_LEVEL_TRACE: memcpy(buffer, "TRACE: ", 8); break;
        case LOG_LEVEL_DEBUG: memcpy(buffer, "DEBUG: ", 8); break;
        case LOG_LEVEL_INFO: memcpy(buffer, "INFO: ", 7); break;
        case LOG_LEVEL_WARNING: memcpy(buffer, "WARNING: ", 10); break;
        case LOG_LEVEL_ERROR: memcpy(buffer, "ERROR: ", 8); break;
        case LOG_LEVEL_FATAL: memcpy(buffer, "FATAL: ", 8); break;
        default: break;
    }
            ...
            while (!feof(raeFile))
            {
                switch (buffer[0])
                {
                    ...
                    case 'e':
                    {
                        ...
                        else TRACELOG(LOG_LEVEL_WARNING, "AUTOMATION: Event goes beyond automated list capacity (MAX: %u): %s", list.capacity, buffer);
                    } break;
                    default: break;
                }
